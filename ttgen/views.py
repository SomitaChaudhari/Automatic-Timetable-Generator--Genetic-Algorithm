...
all_timing = ["9:30 - 10:30", "10:30 - 11:30", "11:30 - 12:30", "12:30 - 1:30", "1:30 - 2:30","2:30 - 3:30", "3:30 - 4:30"]
...
def timetable(request):
    ...
    while population.get_schedules()[0].get_fitness()  != 0.1:
        generation_num += 1
    ...
    for sec in Section.objects.all():
        ...
        for sch in schedule:
            if sec.section_id == sch.section:

                # print()
                # print(sch.section)
                # print(sch.section_id)
                # print(sch.course)
                # print(sch.room)
                # print(sch.instructor)
                # print(sch.meeting_time)
                data = str(sch.meeting_time).split(' ')
                day = str(sch.meeting_time).split(' ')[1]
                time = str(sch.meeting_time).split(' ')[2]
                day_time_dic[day][data[2]+" - "+data[4]] = "Subject: "+str(sch.course) + " Room: " + str(sch.room)
                #  day_time_dic[day][data[2]+" - "+data[4]] = "Subject: "+str(sch.course) + "<br> Room: " + str(sch.room) + "<br> Instructor: "+str(sch.instructor)
        
        final_result[sec.section_id] = day_time_dic


    print(final_result)        


    return render(request, 'gentimetable.html',  {'schedule': final_result, 'final_result':final_result, 'sections': Section.objects.all(),
